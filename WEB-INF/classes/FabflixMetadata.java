...
public class FabflixMetadata extends HttpServlet
{
        ...
        try
	{
		...
		String[] tables = new String[]{"creditcards", "customers", "genres", "genres_in_movies", "movies", "sales", "stars", "stars_in_movies", "employees"};
		
		for (int i = 0; i < tables.length; i++)
		{
			Statement state = dbcon.createStatement();
			ResultSet result;
			result = state.executeQuery("SELECT * FROM " + tables[i] + ";");
			ResultSetMetaData metadata = result.getMetaData();
			out.println(tables[i] + " Table: <br>");
			for (int x = 1; x <= metadata.getColumnCount(); x++)
	        		out.println("\tColumn Name: " + metadata.getColumnName(x)+ "	---	Type: " + metadata.getColumnTypeName(x) + "<br>");
			out.println("<br>");
		}
		dbcon.close();
	}
