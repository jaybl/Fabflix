...
public class FabflixItems extends HttpServlet { 
   public void doGet(HttpServletRequest request,
    ...
    try{
      ...
      String formCheckout = "<FORM ACTION=\"/Fabflix/checkout\" METHOD=\"GET\">";
	...
	out.println("<script type\"text/javascript\"> alert(\"Cannot add to Cart\"); location='/Fabflix/movie?movieID=" + newItem + "';</script>");
      }
      else{
	...
	out.println("</div>");
      ...
      synchronized(items) {
        if (items.containsKey(newItem) && !quantity.equals("")){
          //int x = Integer.parseInt(items.get(newItem));
	  ...
	  items.put(newItem, quantity);
        ...
        /*if (items.containsKey(newItem) && quantity.equals("")){
          int x = Integer.parseInt(items.get(newItem));
	  String sum = Integer.toString(x+1);
	  items.put(newItem, sum);
        }
        if(!items.containsKey(newItem) && quantity.equals("")){
          items.put(newItem, "1");
        }*/
	...
	out.println("<div class=\"row\" style=\"text-align: center; padding: 15px;\">");
	    ...
	    out.println("<div style=\"box-shadow: 0 2px 26px 0 rgba(0,0,0,0.18);\">");
	      ...
	      out.println(key + "<br><LI style=\"list-style-type: none;\"> <img src=\"" + imageurl + "\" alt=\"" + m_T + "\" style=\"width:66px;height:86px;\">" 
                          + "<br><BR><b> " 
                          + m_T + ":</b>" + formItems + "<input name=\"quantity\" value=" + items.get(key) + " style=\"width:50px;\">" +
			  "<button style=\"submit\" value="+ key +" name=\"movieID\">update quantity</button></form><br><br><br>");
            }
	    rs.close();
	    out.println("</div>");
          }
          out.println("</UL>");
	  out.println(formCheckout + "<button type=\"SUBMIT\" style=\"margin-top: 25px; background-color: white; font-weight: bold; border-radius: 3px; border: 1px solid #007EE5; padding: 10px; padding-left: 15px; padding-right: 15px;\">Checkout</button></form>");
        }
      }
      out.println("</div>");
